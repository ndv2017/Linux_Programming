...
static void reassign_connection_ids() {
    int write_index = 0;

    /* Compact the array by moving active connections to the front */
    for (int read_index = 0; read_index < MAX_PEERS; read_index++) {
        if (peers[read_index].active) {
            if (read_index != write_index) {
                /* Move the connection to the new position */
                peers[write_index] = peers[read_index];

                /* Clear the old position */
                peers[read_index].socket_fd = -1;
                peers[read_index].active = 0;
                memset(peers[read_index].ip, 0, INET_ADDRSTRLEN);
                peers[read_index].connection_port = 0;
                peers[read_index].listening_port = 0;
            }
            write_index++;
        }
    }
    ...
    for (int i = write_index; i < MAX_PEERS; i++) {
        peers[i].socket_fd = -1;
        peers[i].active = 0;
        memset(peers[i].ip, 0, INET_ADDRSTRLEN);
        peers[i].connection_port = 0;
        peers[i].listening_port = 0;
    }
}
